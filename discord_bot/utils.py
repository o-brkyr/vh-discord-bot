...
from discord import Role, Guild, Colour, TextChannel
...
async def get_or_create_valheimer_channel(guild: Guild) -> TextChannel:
    ...
    """
    if (channel := discord_utils.get(guild.channels, name=CHANNEL_TITLE)) is None:
...
async def get_or_create_valheimer_role(guild: Guild) -> Role:
    ...
    if (role := discord_utils.get(guild.roles, name=ROLE_NAME)) is None:
        role = await guild.create_role(
            name=ROLE_NAME, colour=Colour.dark_gold, mentionable=True, reason="Because"
        )
    return role
